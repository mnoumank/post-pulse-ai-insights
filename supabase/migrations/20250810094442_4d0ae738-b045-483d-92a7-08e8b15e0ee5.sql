...
drop policy if exists "Users can view their own actuals" on public.comparison_actuals;
create policy "Users can view their own actuals"
  on public.comparison_actuals for select
  using (auth.uid() = user_id);

drop policy if exists "Users can insert their own actuals" on public.comparison_actuals;
create policy "Users can insert their own actuals"
  on public.comparison_actuals for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can update their own actuals" on public.comparison_actuals;
create policy "Users can update their own actuals"
  on public.comparison_actuals for update
  using (auth.uid() = user_id);

drop policy if exists "Users can delete their own actuals" on public.comparison_actuals;
